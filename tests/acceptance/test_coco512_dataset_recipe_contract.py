...
def test_coco512_dataset_recipe_points_to_larger_smoke_subset() -> None:
    request = load_dataset_import_request("configs/datasets/coco2017_smoke_512.yaml")

    assert request.dataset_id == "phiyodr/coco2017"
    assert request.split == "train"
    assert request.output_root == Path("data/coco2017_smoke_512")
